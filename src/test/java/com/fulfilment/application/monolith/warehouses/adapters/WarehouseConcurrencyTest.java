...
public class WarehouseConcurrencyTest {
  ...
  @Transactional
  public void cleanUp() {
    warehouseRepository.delete("businessUnitCode like ?1", "CONCURRENT-%");
    warehouseRepository.delete("businessUnitCode like ?1", "DUPLICATE-%");
    warehouseRepository.delete("businessUnitCode like ?1", "READ-TEST-%");
  }
      ...
      executor.submit(() -> {
        try {
          io.quarkus.narayana.jta.QuarkusTransaction.requiringNew().run(() -> {
            ...
            warehouse.stock = 5;
            createWarehouseUseCase.create(warehouse);
            warehouseRepository.flush(); // Force DB hit to trigger ConstraintViolation exception
